...
public struct KATrackFloat {
    ...
    private func customHermiteInterpolation(curve: CubicHermiteFloat, t: Float) -> Float {
        let tt = t * t
        let ttt = tt * t
    ...
    func sampleConstant(t: Float, looping: Bool) -> Float {
    ...
    func sampleLinear(t: Float, looping: Bool) -> Float {
    ...
    func sampleCubic(t: Float, looping: Bool) -> Float {
        let currentFrame = frameIndex(t: t, looping: looping)
        if currentFrame < .zero || currentFrame >= (frames.count - 1) {
            return .zero
        }
