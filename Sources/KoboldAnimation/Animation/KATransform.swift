...
public struct KATransform: Equatable {
    ...
    public init(
        ...
        rotation: simd_quatf = simd_quatf(real: 1, imag: SIMD3<Float>(repeating: 0)),
    ...
    func combine(_ other: KATransform) -> KATransform {
        return KATransform(
    ...
    func lerp(_ other: KATransform, t: Float) -> KATransform {
...
extension KATransform {
    init(fromMatrix m: simd_float4x4) {
        self.position = m[3].xyz
        self.rotation = KATransform.mat4ToQuat(m)
        let rotationScaleMatrix = m.toRotationScaleMatrix()
        let inverseRotationMatrix = KATransform.quatToMat4(self.rotation.inverse)
